...
def process_row(index, row):
    ...
    time.sleep(1) # Extra polite delay
    return index, result

def run_smart_enrichment():
    print("=== SMART AI ENRICHMENT (PERPLEXITY) ===")
    ...
    if len(targets) == 0:
        ...
        targets = df[missing_dm].head(200) # Process next 200
        print(f"Fallback: Processing next {len(targets)} missing DM leads.")
    ...
    df.to_csv(OUTPUT_FILE, index=False)
    print(f"🎉 Done. Enriched {found_count} leads with Decision Makers.")
