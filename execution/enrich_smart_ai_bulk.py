...
OUTPUT_FILE = "/Users/rafaelalmeida/lifetrek-mirror/MASTER_ENRICHED_LEADS.csv" # Overwrite
MODEL = "sonar"
MAX_WORKERS = 2
...
def run_smart_enrichment_bulk():
    print("=== SMART AI ENRICHMENT (BULK) ===")
    ...
    # Sort by Score Descending to process best leads first
    targets = df[missing_dm].sort_values('V2_Score', ascending=False).head(500)
    
    print(f"Found {len(targets)} Targets (Missing DM, Top 500 by Score)")
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if res:
                ...
                if dms and len(dms) > 0:
                    df.at[idx, 'Decision_Maker'] = "; ".join(dms[:2])
                    updates.append("DM")
                    found_count += 1
                    
                if li_company and 'linkedin.com' in str(li_company):
                     curr_li = df.at[idx, 'LinkedIn_Company']
                     if pd.isna(curr_li) or str(curr_li) == '':
                         df.at[idx, 'LinkedIn_Company'] = li_company
                         updates.append("LI")
                
                if updates:
                    print(f"   ✅ Upd [{idx}]: {', '.join(updates)}")
            
            # Save every 20
            if i > 0 and i % 20 == 0:
                df.to_csv(OUTPUT_FILE, index=False)
                print(f"💾 Checkpoint. Found DMs: {found_count}")

    df.to_csv(OUTPUT_FILE, index=False)
    print(f"🎉 Done. Enriched {found_count} leads.")

if __name__ == "__main__":
    run_smart_enrichment_bulk()
