"""
Enrich New Leads (from Google Places)
Uses Apify Cheerio Scraper
"""
...
# Load New Leads
df = pd.read_csv('/Users/rafaelalmeida/lifetrek-mirror/new_leads_google_places.csv')
...
print(f"=== ENRICHING NEW LEADS ===")
print(f"Total leads: {len(df)}")
print(f"Valid websites: {len(pending)}")

# Deduplicate by URL
pending = pending.drop_duplicates(subset=['Website'])
print(f"Unique websites: {len(pending)}")
...
print(f"\n🚀 Launching Scraper for {len(start_urls)} sites...")
try:
