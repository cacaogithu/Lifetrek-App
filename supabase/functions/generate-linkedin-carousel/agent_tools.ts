...
export async function generateCarouselEmbedding(
  ...
  try {
    if (!genAI) {
      console.warn("⚠️ Embedding: No API key available, skipping");
      return null;
    }

    // Combine carousel content into searchable text
    const carouselText = [
      `Topic: ${topic}`,
      ...slides.map((s, i) => `Slide ${i + 1}: ${s.headline} - ${s.body}`)
    ].join('\n');

    console.log("🔢 Generating embedding for carousel content...");
    console.log(`🔢 Embedding input length: ${carouselText.length} chars`);

    // Use Google's text embedding model
    // Note: text-embedding-004 is the latest embedding model from Google AI
    const model = genAI.getGenerativeModel({ model: "text-embedding-004" });
    console.log("🔢 Calling embedContent...");

    const result = await model.embedContent(carouselText);
    console.log("🔢 embedContent result:", result ? "received" : "null");

    if (!result || !result.embedding) {
      console.error("❌ Embedding: No embedding in result", result);
      return null;
    }

    const embedding = result.embedding.values;
    console.log(`✅ Generated ${embedding?.length || 0}d embedding`);
    return embedding;
