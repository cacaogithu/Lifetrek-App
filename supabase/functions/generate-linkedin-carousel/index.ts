...
serve(async (req: Request) => {
  ...
  // Capture auth header EARLY. The request object can be closed by the client
  // during long-running generations, and reading headers later may throw.
...
- Professional, subtle integration` },
    ...
    // Resolve admin user ONCE (do not read req.headers here; the request can be closed).
    ...
    try {
      ...
      console.warn("Auth user lookup failed (non-fatal):", e);
    }
    ...
    if (!adminUserId) {
      ...
      if (fallbackErr) {
        console.warn("Could not fetch fallback admin user:", fallbackErr.message);
      }
      adminUserId = fallbackAdmin?.user_id ?? null;
    }

    if (!adminUserId) {
      throw new Error("No admin user available to own generated carousels");
    }

    for (const carousel of (carouselsToSave as any[])) {
      try {
        // 1. Insert into linkedin_carousels with ALL required fields
        const normalizedSlides = Array.isArray(carousel.slides)
          ? carousel.slides
          : (carousel.slides?.slides || []);

        const normalizedImageUrls =
          carousel.imageUrls ||
          normalizedSlides.map((s: any) => s.imageUrl).filter(Boolean) ||
          [];
