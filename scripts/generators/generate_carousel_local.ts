...
const SLIDES = [
    ...
    },
    {
        id: 'cta-final',
        asset: 'SOLID_BLUE',
        headline: 'ACELERE O DESENVOLVIMENTO DO SEU PRODUTO',
        subhead: 'Desenvolva componentes médicos de precisão com nossa fabricação certificada ISO 13485, tecnologia CNC de última geração e instalações de sala limpa.',
        body: '',
        type: 'cta'
    }
];
...
async function generateSlide(slide: any, index: number) {
    ...
    let baseImage;
    if (slide.asset === 'SOLID_BLUE') {
        baseImage = await sharp({
            create: {
                width: WIDTH,
                height: HEIGHT,
                channels: 4,
                background: { r: 0, g: 79, b: 143, alpha: 1 } // #004F8F
            }
        }).png().toBuffer();
    } else {
        ...
        baseImage = await sharp(assetPath)
    ...
    // Alternate text position: even slides (0, 2) at TOP now (isBottom = false), odd at BOTTOM
